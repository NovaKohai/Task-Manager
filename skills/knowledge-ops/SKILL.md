---
name: knowledge-ops
description: Use when a Head of Ops, Knowledge Manager, or TPM-Internal needs to author, validate, or clean up company SOPs and internal runbooks — including 5W2H completeness checks, cross-link and orphan-page validation across Notion/Confluence/Obsidian wiki, KB ingestion + hygiene reporting, and runbook step verification (named owner, duration, success signal, rollback path). Pairs Kaoru Ishikawa's 5W2H method, Atul Gawande's *The Checklist Manifesto*, ISO 9001, ITIL v4, FDA 21 CFR Part 211, and Google SRE Workbook with stdlib-only Python tools scoring completeness, detecting anti-patterns, and emitting prioritized cleanup lists. Distinct from `engineering/llm-wiki` (PKM), `engineering-team/runbook-generator` (debugging), `project-management/*` (delivery), and sibling `business-operations/process-mapper` (BPMN design vs documentation).
context: fork
version: 2.8.0
